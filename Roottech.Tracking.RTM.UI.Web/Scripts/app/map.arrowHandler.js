...
ArrowHandler.prototype.load = function (points, mode, index) {
    for (var i = 0; i < points.length - 1; i++) {
        var p1 = points[i],
      p2 = points[i + 1];
        this.create(p1, p2, mode);
    }
    if (overlaysArray.marker[index] != null) 
        this.arrowheads.push(overlaysArray.marker[index]);
    overlaysArray.marker[index] = this.arrowheads;
};