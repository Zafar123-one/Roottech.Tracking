...
(function ($) {
    jQuery.fn.contextMenu = function(name, actions, options) {
        var me = this,
            win = $(window),
            menu = $('<ul id="' + name + '" class="rightClickOptions" ></ul>').hide().appendTo('body'),
            activeElement = null, // last clicked element that responds with contextMenu
            hideMenu = function() {
                $('.rightClickOptions:visible').each(function () {
                    $(this).trigger("closed");
                    $(this).hide();
        ...
        $.each(actions, function (me, itemOptions) {
            if (itemOptions.hide) return; 
            if (itemOptions.link) {
                var link = itemOptions.link;
            } else {
                var link = '<a href="#"><span class="left ic"><img src="' + $("#imagePath").prop("class") + itemOptions.image + '"></span><span>' + me + '</span></a>';
            }
        ...
        var mouseEventFunc = function(e) {
            ...
            if (correctButton) {
                ...
                menu.show(0, function() {
                    ...
                    top: yPos + 'px',
                    left: xPos + 'px',
                    zIndex: 99999 /* replace z-index value to clicked element value plus 1*/
                });
