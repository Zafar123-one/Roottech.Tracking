	...
	$.fn.speedometer = function( options ){
		...
		var def = {
			...
			thisCss: {
				...
				fontFamily: 'Arial',
				fontWeight: '900',
				backgroundImage : "url('" + $("#scriptPath").prop("class") + "speedometer/background.jpg')"
		...
		if ( def.limit && ( def.percentage > def.maximum || def.percentage < def.minimum ) ) {
			/* the glass cracks */
			$this.css( 'backgroundImage' , 'url("' + $("#scriptPath").prop("class") + 'speedometer/background-broken.jpg")' );
		} else {
